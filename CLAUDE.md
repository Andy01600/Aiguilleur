...
| 2026-04-04 | Critère 3bis — pénibilité du repli dans cle_priorite() | Problème : l'équipe la plus proche gagne toujours le voeu 1, même si son repli est confortable, tandis qu'une équipe à peine plus loin se retrouve envoyée très loin. Solution : ajouter un critère de départage entre distance (3) et horodatage (4). Pénibilité = distance(équipe → repli objectif) − distance(équipe → compétition disputée). Le repli objectif = compétition viable la plus proche, excluant la compétition disputée et les compétitions en conflit vacances SAUF si l'équipe a voté pour une compétition tombant dans la même période de vacances (signal de disponibilité sur toute la période). Ne modifie pas l'algo existant, s'insère dans le tuple de `cle_priorite()`. |
